...
首次运行会自动：建 `venv/` → 按 GPU 驱动检测装对应 CUDA torch（cu118 至 cu130）→ 装 `requirements.txt` → 按 GPU 检测装 onnxruntime → 构建前端 → 起后端 → 自动开浏览器到 <http://127.0.0.1:8766/studio/>。
...
python -m studio dev          # 前后端 watch：vite 5173 + uvicorn 8766 --reload
